...
class ProgressTrackerV2:
    ...
    def __init__(self, total_files: int, total_size: int):
        ...
        # v2 新機能: 性能統計
        self.max_speed = 0.0  # 最高速度記録
        self.file_completion_times = []  # ファイル完了時刻記録（スループット計算用）
        
        print(f"\n{'='*80}")
        print(f"📊 アーカイブ処理開始 (v2 - 詳細統計付き)")
    ...
    def update_file_failure(self, file_path: str, file_size: int, error_msg: str):
        ...
        self._display_progress(f"失敗: {error_type}")
    ...
    def _classify_error(self, error_msg: str) -> str:
        ...
        # 権限エラー
        if any(keyword in error_lower for keyword in ["permission", "access", "権限", "アクセス"]):
            return "権限エラー"
        
        # ネットワークエラー
        elif any(keyword in error_lower for keyword in ["network", "connection", "timeout", "ネットワーク", "接続"]):
            return "ネットワークエラー"
    ...
    def _calculate_throughput(self) -> float:
        ...
        if len(recent_completions) > 0:
            ...
            if time_span > 0:
                return (len(recent_completions) / time_span) * 60  # ファイル数/分
...
class ArchiveProcessorTestV2:
    ...
    def setup_logger(self) -> logging.Logger:
        """ログ設定の初期化"""
        logger = logging.getLogger('archive_processor_test_v2')
        ...
        try:
            ...
            log_file = log_dir / f"archive_test_v2_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    def archive_to_s3(self, files: List[Dict]) -> List[Dict]:
        ...
        # v2 進捗トラッカー初期化
        total_size = sum(f['size'] for f in files)
        self.progress_tracker = ProgressTrackerV2(len(files), total_size)
    ...
    def run(self, csv_path: str, request_id: str) -> int:
        ...
        try:
            print(f"🚀 アーカイブ処理開始 (v2) - Request ID: {request_id}")
...
def main():
    """メイン関数"""
    parser = argparse.ArgumentParser(description='アーカイブスクリプト検証版v2（進捗表示精度向上版）')
    ...
    print(f"🔍 アーカイブスクリプト検証版 v2")
    print(f"📋 機能: エラー分類・性能統計・詳細進捗表示")
    print(f"📄 CSV: {args.csv_path}")
    print(f"🆔 Request ID: {args.request_id}")
    print(f"⚙️  設定ファイル: {args.config}")
    
    # アーカイブ処理の実行
    processor = ArchiveProcessorTestV2(args.config)
