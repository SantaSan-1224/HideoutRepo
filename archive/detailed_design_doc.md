...
### 1.1 システム目的
企業内ファイルサーバ（FSx for Windows File Server）上のファイルを、ユーザー依頼に基づいてAWS S3（Glacier Deep Archive）にアーカイブし、履歴管理・復元機能を提供する。
...
| request_id | VARCHAR(50) | NOT NULL | 依頼ID |
| requester | VARCHAR(7) | NOT NULL, CHECK | 依頼者（社員番号7桁） |
...
#### 3.3.2 エラーCSV出力仕様
**CSV検証エラー**: `{元ファイル名}_csv_errors_{YYYYMMDD_HHMMSS}.csv`
```csv
行番号,パス,エラー理由,元の行
2,\\invalid\path,ディレクトリが存在しません,\\invalid\path
```
...
#### 4.2.3 エクスポート機能
- **Excel形式**: `pandas.to_excel()`使用
- **CSV形式**: `pandas.to_csv()`使用
...
### 7.2 データ保護
- **通信暗号化**: VPCエンドポイント経由のHTTPS通信
- **データベース接続**: SSL接続（設定による）
- **ログ保護**: 機密情報のマスキング
...
### 9.2 トラブルシューティング
- **CSV読み込みエラー**: 文字エンコーディング確認
- **S3接続エラー**: VPCエンドポイント・認証情報確認
