...
# Zone判定フラグ定義
# 0 -> TeijinZone (Default) , 1 -> OracleZone
ZONE_FLG=${ZONE_FLG:-0};readonly ZONE_FLG
...
fDisableSubscription () {

	local RC=255
	local SUBSCRIPTION_MGR_CNF=/etc/yum/pluginconf.d/subscription-manager.conf
	...
	if [ ! -f ${SUBSCRIPTION_MGR_CNF} ];then
		fOutputLog "${SUBSCRIPTION_MGR_CNF}が存在しません"
		return ${RC}
	fi

	grep 'enabled=0' ${SUBSCRIPTION_MGR_CNF} >/dev/null 2>&1
	RESULT=$?
	if [ ${RESULT} -ne 0 ];then
		fOutputLog "${SUBSCRIPTION_MGR_CNF} を変更します"
		sed -i -e 's/enabled=./enabled=0/' ${SUBSCRIPTION_MGR_CNF}
		RC=0
	else
		fOutputLog "${SUBSCRIPTION_MGR_CNF} : OK"
		RC=0
	fi
